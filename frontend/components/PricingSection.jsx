...
export default function PricingSection({ subscriptionTier = "free" }) {
  ...
  // If plan ID is not configured, show a message
  const isPlanConfigured = !!proPlanId && proPlanId.trim().length > 0;
  return (
          ...
          <CardFooter>
            <SignedIn>
              ...
              {isPlanConfigured ? (
                ...
                <Button
                  disabled
                  className="w-full bg-red-400 cursor-not-allowed text-white"
                  title="Plan not configured. Please contact support."
                >
                  Plan not available
                </Button>
              )}
